...
_PROGRAMS = [
  ...
  (func (export "run") (result i32)
    ...
    ("gc-array-payload-rooted-through-unwind", "f", "OK 1237", '''(module
  (type $payload (array (mut i32)))
  (type $scratch (array (mut i32)))
  (tag $t (param (ref $payload)))
  (func $gc (local $i i32)
    (block $done (loop $l
      (br_if $done (i32.ge_u (local.get $i) (i32.const 4000)))
      (drop (array.new_default $scratch (i32.const 1000)))
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (br $l))))
  (func $mk (result (ref $payload))
    (local $p (ref $payload))
    (local.set $p (array.new_default $payload (i32.const 4)))
    (array.set $payload (local.get $p) (i32.const 0) (i32.const 1000))
    (array.set $payload (local.get $p) (i32.const 1) (i32.const 200))
    (array.set $payload (local.get $p) (i32.const 2) (i32.const 30))
    (array.set $payload (local.get $p) (i32.const 3) (i32.const 7))
    (local.get $p))
  (func (export "f") (result i32)
    (local $p (ref null $payload))
    (block $caught (result (ref $payload))
    ...
    (call $gc)
    (i32.add
      (i32.add (array.get $payload (ref.as_non_null (local.get $p)) (i32.const 0))
               (array.get $payload (ref.as_non_null (local.get $p)) (i32.const 1)))
      (i32.add (array.get $payload (ref.as_non_null (local.get $p)) (i32.const 2))
               (array.get $payload (ref.as_non_null (local.get $p)) (i32.const 3))))))'''),

    ("exnref-held-payload-across-gc-rethrow", "f", "OK 271828", '''(module
  (type $inner (array (mut i32)))
  (type $box (struct (field (ref $inner))))
  (type $scratch (array (mut i32)))
  (tag $t (param (ref $box)))
  (func $gc (local $i i32)
    (block $done (loop $l
      ...
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (br $l))))
  (func $mk (result (ref $box))
    (local $a (ref $inner))
    (local.set $a (array.new_default $inner (i32.const 3)))
    (array.set $inner (local.get $a) (i32.const 1) (i32.const 271828))
    (struct.new $box (local.get $a)))
  (func (export "f") (result i32)
    (local $e (ref null exn))
    (block $cap (result (ref $box) exnref)
      (try_table (catch_ref $t $cap)
        (throw $t (call $mk)))
      (unreachable))
    (local.set $e)
    (drop)
    (call $gc)
    (block $re (result (ref $box))
      (try_table (catch $t $re)
        (throw_ref (ref.as_non_null (local.get $e))))
      (unreachable))
    (array.get $inner (struct.get $box 0) (i32.const 1))))'''),
