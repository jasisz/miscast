...
```
python3 -m miscast --sut ENGINE [--mode replay|mutate|smith|morphism|recgroup|compose|trapline|invalid|all] [--oracles LIST] [--seeds DIR] [-n N] [--overtrap]
```
...
`recgroup` / `compose` / `trapline` / `invalid` / `wast` / `reduce` / `modes` / `repro` /
