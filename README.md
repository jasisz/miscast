...
```
python3 -m miscast --sut ENGINE [--mode replay|mutate|smith|morphism|recgroup|compose|invalid|all] [--oracles LIST] [--seeds DIR] [-n N] [--overtrap]
```
...
`recgroup` / `compose` / `invalid` / `wast` / `reduce` / `modes` / `repro` /
