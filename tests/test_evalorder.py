"""Unit tests for the side-effecting operand-order oracle."""
import os
import shutil
import subprocess
import sys
...
from miscast.evalorder import evalorder_gen, _FAMILIES
from miscast.modes import gen_evalorder
...
def _assemble(module):
    p = subprocess.run(["wasm-tools", "parse", "/dev/stdin", "-o", "/tmp/_eo_test.wasm"],
                       input=module.encode(), capture_output=True)
    assert p.returncode == 0, f"evalorder module is not valid wat: {p.stderr.decode()[:200]}"


def _run_wasmtime(export):
    r = subprocess.run(["wasmtime", "run", "-W", "function-references=y,gc=y,exceptions=y,tail-call=y",
                        "--invoke", export, "/tmp/_eo_test.wasm"],
...
def test_assembles_and_self_checks():
    ...
    for seed in range(2 * len(_FAMILIES)):
        label, export, expected, wat = evalorder_gen(seed)
        _assemble(wat)
        if has_wasmtime:
