"""Unit tests for the runtime packed-GC storage oracle."""
import os
import shutil
import subprocess
import sys
...
from miscast.packedops import packedops_gen, _FAMILIES
from miscast.modes import gen_packedops
...
def _assemble(module):
    p = subprocess.run(["wasm-tools", "parse", "/dev/stdin", "-o", "/tmp/_po_test.wasm"],
                       input=module.encode(), capture_output=True)
    assert p.returncode == 0, f"packedops module is not valid wat: {p.stderr.decode()[:200]}"


def _run_wasmtime(export):
    r = subprocess.run(["wasmtime", "run", "-W", "gc=y", "--invoke", export, "/tmp/_po_test.wasm"],
...
def test_assembles_and_self_checks():
    ...
    for seed in range(4 * len(_FAMILIES)):
        label, export, expected, wat = packedops_gen(seed)
        _assemble(wat)
        if has_wasmtime:
