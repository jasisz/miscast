"""Unit tests for the reference-identity oracle."""
import os
import shutil
import subprocess
import sys
...
def _assemble(module):
    p = subprocess.run(["wasm-tools", "parse", "/dev/stdin", "-o", "/tmp/_ra_test.wasm"],
                       input=module.encode(), capture_output=True)
    assert p.returncode == 0, f"refalias module is not valid wat: {p.stderr.decode()[:200]}"


def _run_wasmtime(export):
    r = subprocess.run(["wasmtime", "run", "-W", "function-references=y,gc=y,exceptions=y",
                        "--invoke", export, "/tmp/_ra_test.wasm"],
                       capture_output=True, text=True)
    both = (r.stdout + r.stderr).lower()
    if r.returncode != 0 or any(k in both for k in ("trap", "unreachable", "null", "runtimeerror")):
        return "TRAP"
...
def test_mode_cases():
    ...
    eq("case is prefixed", nm.startswith("refalias0|refalias-"), True)
    eq("export is f", export, "f")
    eq("no invoke args", args, [])
    eq("expected is baked", expected in ("OK 0", "OK 1"), True)
...
def test_assembles_and_self_checks():
    ...
    for seed in range(2 * len(_FAMILIES)):
        label, export, expected, wat = refalias_gen(seed)
        _assemble(wat)
        if has_wasmtime:
