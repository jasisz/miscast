"""Unit tests for the stateful heap-storm oracle."""
import os
import shutil
import subprocess
import sys
...
def _assemble(module):
    p = subprocess.run(["wasm-tools", "parse", "/dev/stdin", "-o", "/tmp/_hs_test.wasm"],
                       input=module.encode(), capture_output=True)
    assert p.returncode == 0, f"heapstorm module is not valid wat: {p.stderr.decode()[:200]}"


def _run_wasmtime(export):
    r = subprocess.run(["wasmtime", "run", "-W", "function-references=y,gc=y,exceptions=y,tail-call=y",
                        "--invoke", export, "/tmp/_hs_test.wasm"],
...
def test_mode_cases():
    ...
    eq("case is prefixed", nm.startswith("heapstorm0|heapstorm-"), True)
...
def test_assembles_and_self_checks():
    ...
    for seed in range(2 * len(_FAMILIES)):
        label, export, expected, wat = heapstorm_gen(seed)
        _assemble(wat)
        if has_wasmtime:
