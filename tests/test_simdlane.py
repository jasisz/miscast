"""Unit tests for the SIMD lane-algebra oracle."""
import os
import shutil
import subprocess
import sys
...
def _assemble(module):
    p = subprocess.run(["wasm-tools", "parse", "/dev/stdin", "-o", "/tmp/_sl_test.wasm"],
                       input=module.encode(), capture_output=True)
    assert p.returncode == 0, f"simdlane module is not valid wat: {p.stderr.decode()[:200]}"


def _run_wasmtime(export):
    r = subprocess.run(["wasmtime", "run", "--invoke", export, "/tmp/_sl_test.wasm"],
...
def test_mode_cases():
    ...
    eq("case is prefixed", nm.startswith("simdlane0|simdlane-"), True)
...
def test_assembles_and_self_checks():
    ...
    for seed in range(3 * len(_FAMILIES)):
        label, export, expected, wat = simdlane_gen(seed)
        _assemble(wat)
        if has_wasmtime:
